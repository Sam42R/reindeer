...
public class CircleTextLayer implements MissionPatchLayer {
    ...
    public List<MissionPatchLayerProperty<?>> properties() {
        return List.of(
                ...
                new MissionPatchLayerProperty<>("start", Integer.class, this::getStart, this::setStart),
                new MissionPatchLayerProperty<>("end", Integer.class, this::getEnd, this::setEnd),
    ...
    public void draw(Canvas canvas) {
        ...
        var numRadsPerLetter = ((Math.abs(start) + Math.abs(end)) * Math.PI / 180) / (text.length() - 1);
        ctx.save();
        ctx.translate(x, y);
        ctx.rotate(start * Math.PI / 180);
        ...
        for (var i = 0; i < text.length(); i++) {
            ...
            ctx.fillText(String.valueOf(text.charAt(i)), 0, -radius);
            ctx.restore();
        }
        ctx.restore();
    }
